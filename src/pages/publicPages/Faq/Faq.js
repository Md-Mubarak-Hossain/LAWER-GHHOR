    ...
    return (
        <div className='p-2'>
            ...
            <div className='mx-auto w-11/12 m-5 border border-primary p-4 rounded ' ref={ref}>
                ...
                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify">The private route component is similar to the public route, the only change is that redirect URL and authenticate condition. If the user is not authenticated he will be redirected to the login page and the user can only access the authenticated routes If he is authenticated (Logged in).</p>
                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify">
            ...
            <div className='mx-auto w-11/12 m-5 border border-primary p-4 rounded ' ref={ref}>
                ...
                <li>Asynchronus</li>
                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify"> Non-blocking I/o: Non-blocking i/o  means working with multiple requests without blocking the thread for a single request. I/O basically interacts with external systems such as files, databases. Node.js is not used for CPU-intensive work means for calculations, video processing because a single thread cannot handle the CPU works.</p>


                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify"> Asynchronous: Asynchronous is executing a callback function. The moment we get the response from the other server or database it will execute a callback function. Callback functions are called as soon as some work is finished and this is because the node.js uses an event-driven architecture. The single thread doesn’t work with the request instead it sends the request to another system which resolves the request and it is accessible for another request.</p>

                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify"> To implement the concept of the system to handle the request  node.js uses the concept of Libuv.</p>


                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify"> Libuv is an open-source library built-in C. It has a strong focus on asynchronous and  I/O, this gives node access to the underlying computer operating system, file system, and networking.</p>

                <h2 className='text-bold text-warning text-3xl'>Libuv implements two extremely important features of node.js</h2>

                <li>Event loop</li>
                <li> Thread pool</li>
                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify"> Event loop: The event loop contains a single thread and is responsible for handling easy tasks like executing callbacks and network I/O. When the program is to initialize all the top-level code is executed, the code is not in the callback function. All the applications code that is inside callback functions will run in the event loop. EventLoop is the heart of node.js. When we start our node application the event loop starts running right away. Most of the work is done in the event loop.</p>

                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify">Nodejs use event-driven-architecture.</p>


                <li> Events are emitted.</li>
                <li>Event loop picks them up.</li>
                <li> Callbacks are called.</li>
                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify"> Event queue: As soon as the request is sent the thread places the request into a queue. It is known as an event queue. The process like app receiving HTTP request or server or a timer will emit event as soon as they are done with the work and event loop will pick up these events and call the callback functions that are associated with each event and response is sent to the client.</p>

                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify">The event loop is an indefinite loop that continuously receives the request and processes them. It checks the queue and waits for the incoming request indefinitely.</p>

                <p className="p-4 text-semibold    my-2 rounded text-xl text-justify">Thread pool: Though node.js is single-threaded it internally maintains a thread pool. When non-blocking requests are accepted there are processed in an event loop, but while accepting blocking requests it checks for available threads in a thread pool, assigns a thread to the client’s request which is then processed and send back to the event loop, and response is sent to the respective client.</p>
